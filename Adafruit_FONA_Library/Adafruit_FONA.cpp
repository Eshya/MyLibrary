...
uint8_t Adafruit_FONA::setNetwork(void) {
  uint16_t reply;

  if (!sendParseReply(F("AT+CREG=1"), F(""), &reply))
    return 0;
...
bool Adafruit_FONA::sendUSSD(char *ussdmsg, char *ussdbuff, uint16_t maxlen,
  ...
  if (!sendCheckReply(sendcmd, ok_reply)) {
    *readlen = 0;
    ...
    readline(5000); // read the +CUSD reply, wait up to 10 seconds!!!
    DEBUG_PRINT("* "); DEBUG_PRINTLN(replybuffer);
    ...
    uint16_t lentocopy = maxlen - 1;
    DEBUG_PRINT("lentocopy "); DEBUG_PRINTLN(lentocopy);
    DEBUG_PRINT("maxlen "); DEBUG_PRINTLN(strend);
    DEBUG_PRINT("strend - p "); DEBUG_PRINTLN(p);
    strncpy(ussdbuff, p, lentocopy + 1);
    ussdbuff[lentocopy] = 0;
    // DEBUG_PRINT("ussdbuff "); DEBUG_PRINTLN(ussdbuff);
    *readlen = lentocopy;
  }
  return true;
}
