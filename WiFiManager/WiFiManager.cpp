...
WiFiManagerParameter::WiFiManagerParameter(char *custom) {
  _id             = NULL;
  _label          = NULL;
  _length         = 1;
  _value          = NULL;
...
WiFiManagerParameter::WiFiManagerParameter(const char *id, const char *label, const char *defaultValue, int length, char *custom) {
  init(id, label, defaultValue, length, custom, WFM_LABEL_BEFORE);
}
WiFiManagerParameter::WiFiManagerParameter(const char *id, const char *label, const char *defaultValue, int length,  char *custom,const char *id2, const char *label2, const char *defaultValue2, int length2) {
  init2(id, label, defaultValue, length, custom, id2,label2,defaultValue2,length2);
}

WiFiManagerParameter::WiFiManagerParameter(const char *id, const char *label, const char *defaultValue, int length, char *custom, int labelPlacement) {
  init(id, label, defaultValue, length, custom, labelPlacement);
}

void WiFiManagerParameter::init(const char *id, const char *label, const char *defaultValue, int length,char *custom, int labelPlacement) {
  _id             = id;
  _label          = label;
...
void WiFiManagerParameter::init2(const char *id, const char *label, const char *defaultValue, int length, char *custom,const char *id2, const char *label2, const char *defaultValue2, int length2){
  _id             = id;
  _label          = label;
  _labelPlacement = WFM_LABEL_BEFORE;
  _customHTML     = custom;
  ...
  _id2             = id2;
  _label2          = label2;
  setValue2(defaultValue2,length2);
}
...
void WiFiManagerParameter::setValue2(const char *defaultValue2, int length2) {
  if(!_id2){
    // Serial.println("cannot set value of this parameter");
    return;
  }
  ...
  _length2 = length2;
  _value2  = new char[_length2 + 1]; 
  memset(_value2, 0, _length2 + 1); // explicit null
  
  if (defaultValue2 != NULL) {
    strncpy(_value2, defaultValue2, _length2);
  }
}
...
const char* WiFiManagerParameter::getValue2() {
  // Serial.println(printf("Address of _value is %p\n", (void *)_value)); 
  return _value2;
}
const char* WiFiManagerParameter::getID2() {
  return _id2;
}
const char* WiFiManagerParameter::getLabel2() {
  return _label2;
}
int WiFiManagerParameter::getValueLength2() {
  return _length2;
}
...
String WiFiManager::getParamOut(){
  ...
  if(_paramsCount > 0){
    ...
    String HTTP_PARAM_temp2 = FPSTR(HTTP_FORM_LABEL);
    HTTP_PARAM_temp += FPSTR(HTTP_FORM_PARAM);
    HTTP_PARAM_temp2 += FPSTR(HTTP_FORM_PARAM2);
    ...
    bool tok2_I = HTTP_PARAM_temp2.indexOf(FPSTR(T_I)) > 0;
    bool tok2_i = HTTP_PARAM_temp2.indexOf(FPSTR(T_i)) > 0;
    bool tok2_i2 = HTTP_PARAM_temp2.indexOf(FPSTR(T_i2)) > 0;
    bool tok2_n = HTTP_PARAM_temp2.indexOf(FPSTR(T_n)) > 0;
    bool tok2_n2 = HTTP_PARAM_temp2.indexOf(FPSTR(T_n2)) > 0;
    bool tok2_p = HTTP_PARAM_temp2.indexOf(FPSTR(T_p)) > 0;
    bool tok2_t = HTTP_PARAM_temp2.indexOf(FPSTR(T_t)) > 0;
    bool tok2_t2 = HTTP_PARAM_temp2.indexOf(FPSTR(T_t2)) > 0;
    bool tok2_l = HTTP_PARAM_temp2.indexOf(FPSTR(T_l)) > 0;
    bool tok2_l2 = HTTP_PARAM_temp2.indexOf(FPSTR(T_l2)) > 0;
    bool tok2_v = HTTP_PARAM_temp2.indexOf(FPSTR(T_v)) > 0;
    bool tok2_v2 = HTTP_PARAM_temp2.indexOf(FPSTR(T_v2)) > 0;
    bool tok2_c = HTTP_PARAM_temp2.indexOf(FPSTR(T_c)) > 0;
     ...
     String pitem;
      switch (_params[i]->getLabelPlacement()) {
        case WFM_LABEL_BEFORE:
          if(_params[i]->getID2() != NULL ){
            pitem = FPSTR(HTTP_FORM_LABEL);
            pitem += FPSTR(HTTP_FORM_PARAM2);
          }
          else{
            pitem = FPSTR(HTTP_FORM_LABEL);
      ...
      if(_params[i]->getID2() != NULL ){
        if(tok2_I)pitem.replace(FPSTR(T_I), (String)FPSTR(S_parampre)+(String)i); // T_I id number
        if(tok2_i)pitem.replace(FPSTR(T_i), _params[i]->getID()); // T_i id name
        if(tok2_i2)pitem.replace(FPSTR(T_i2), _params[i]->getID2()); // T_i id name
        if(tok2_n)pitem.replace(FPSTR(T_n), _params[i]->getID()); // T_n id name alias
        if(tok2_n2)pitem.replace(FPSTR(T_n2), _params[i]->getID2()); // T_n id name alias
        if(tok2_p)pitem.replace(FPSTR(T_p), FPSTR(T_t)); // T_p replace legacy placeholder token
        if(tok2_t)pitem.replace(FPSTR(T_t), _params[i]->getLabel()); // T_t title/label
        if(tok2_t2)pitem.replace(FPSTR(T_t2) , _params[i]->getLabel2()); // T_t title/label
        snprintf(valLength, 5, "%d", _params[i]->getValueLength());
        snprintf(valLength2, 5, "%d", _params[i]->getValueLength2());
        if(tok2_l)pitem.replace(FPSTR(T_l), valLength); // T_l value length
        if(tok2_l2)pitem.replace(FPSTR(T_l2), valLength2); // T_l value length
        if(tok2_v)pitem.replace(FPSTR(T_v), _params[i]->getValue()); // T_v value
        if(tok2_v2)pitem.replace(FPSTR(T_v2), _params[i]->getValue2()); // T_v value
        if(tok2_c)pitem.replace(FPSTR(T_c), _params[i]->getCustomHTML()); // T_c meant for additional attributes, not html, but can stuff
      }
...
void WiFiManager::doParamSave(){
  ...
  if(_paramsCount > 0){
    ...
    for (int i = 0; i < _paramsCount; i++) {
      ...
      String value,value2,check;
      if(server->hasArg(name)) {
        value = server->arg(name);
      } else {
        if(_params[i]->getID2() != NULL){

          value2 = server->arg(_params[i]->getID2());
      ...
      if(_params[i]->getID2() != NULL){
        value2.toCharArray(_params[i]->_value2, _params[i]->_length2+1);
        value.toCharArray(_params[i]->_value, _params[i]->_length2+1);
        DEBUG_WM(DEBUG_VERBOSE,(String)_params[i]->getID2() + ":",value2);
